...
/*****************************************************/
	echo "<td align=\"center\" bgcolor=\"$bgcolor2\"><b>"._WHERE."</b></td>";
/*****************************************************/
...
//		$active = _ACTIVE;
//		$change = _DEACTIVATE;
			$active = "<A HREF=\"".$admin_file.".php?op=ChangeStatus&amp;bid=$bid\"><IMG SRC=\"images/active.gif\" WIDTH=\"15\" HEIGHT=\"15\" ALT=\""._ACTIVE." ("._DEACTIVATE.")\" BORDER=0></A>";
	    } elseif ($active == 0) {
//		$active = "<i>"._INACTIVE."</i>";
//		$change = _ACTIVATE;
			$active = "<A HREF=\"".$admin_file.".php?op=ChangeStatus&amp;bid=$bid\"><IMG SRC=\"images/inactive.gif\" WIDTH=\"15\" HEIGHT=\"15\" ALT=\""._INACTIVE." ("._ACTIVATE.")\" BORDER=0></A>";
	    }
...
/*
	    echo "<td align=\"center\"><font class=\"content\">[ <a href=\"".$admin_file.".php?op=BlocksEdit&amp;bid=$bid\">"._EDIT."</a> | <a href=\"".$admin_file.".php?op=ChangeStatus&amp;bid=$bid\">$change</a> | ";
	    if ($bkey == "") {
	    ...
	    if ($block_act == 0) {
		echo "<a href=\"".$admin_file.".php?op=block_show&bid=$bid\">"._SHOW."</a> ]</font></td></tr>";
	    } else {
		echo ""._SHOW." ]</font></td></tr>";
	    }
*/		echo "<td align=\"center\"><a href=\"".$admin_file.".php?op=BlocksEdit&amp;bid=$bid\"><IMG SRC=\"images/edit.gif\" alt=\""._EDIT."\" title=\""._EDIT."\" WIDTH=\"20\" HEIGHT=\"16\" BORDER=0 HSPACE=1></a>";
	    if ($bkey == "") {
		echo "<a href=\"".$admin_file.".php?op=BlocksDelete&amp;bid=$bid\"><IMG SRC=\"images/delete.gif\" ALT=\""._DELETE."\" title=\""._DELETE."\" WIDTH=\"20\" HEIGHT=\"16\" BORDER=0 HSPACE=1></a>";
	    } elseif ($bkey != "") {
		echo "<IMG SRC=\"images/bdelete.gif\" ALT=\""._DELETE."\" WIDTH=\"20\" HEIGHT=\"16\" BORDER=0 HSPACE=1>";
	    }
	    if ($block_act == 0) {
		echo "<a href=\"".$admin_file.".php?op=block_show&bid=$bid\"><IMG SRC=\"images/info.gif\" ALT=\""._SHOW."\" title=\""._SHOW."\" WIDTH=\"16\" HEIGHT=\"16\" BORDER=0 HSPACE=1></a></td></tr>";
	    } else {
		echo "<IMG SRC=\"images/binfo.gif\" ALT=\""._SHOW."\" WIDTH=\"16\" HEIGHT=\"16\" BORDER=0 HSPACE=1></td></tr>";
	    }
	}
	echo "</table>"
	    ."<br><br>"
	...
	while ($row = $db->sql_fetchrow($result)) {
	$dir = $row['title'];
	if(is_dir("modules/$dir/blocks")){
	$blocksdir = dir("modules/$dir/blocks");
	while($func=$blocksdir->read()) {
	    if(substr($func, 0, 6) == "block-") {
    		$blockslist .= "$func ";
	    }
...
/* Addon - Conditional Blocks v.1.1.1          START */
/*****************************************************/
function BlocksAdd($title, $content, $url, $bposition, $active, $refresh, $headline, $blanguage, $blockfile, $view, $groups, $expire, $action, $subscription, $display) {
/*****************************************************/
/* Addon - Conditional Blocks v.1.1.1            END */
/*****************************************************/
...
/* Addon - Conditional Blocks v.1.1.1            END */
/*****************************************************/
        echo"<option value=\"4\" $sel5>"._MVGROUPS."</option>"
        ."</select></td></tr><tr><td nowrap>"
        ."<b>"._WHATGROUPS."</b></td><td><font class='tiny'>"._WHATGRDESC."</font><br><select name='groups[]' multiple size='5'>";
...
/* Module - NSN Groups v.1.6.2                 START */
/*****************************************************/
function BlocksEditSave($bid, $bkey, $title, $content, $url, $oldposition, $bposition, $active, $refresh, $weight, $blanguage, $blockfile, $view, $groups, $expire, $action, $subscription, $display) {
    global $prefix, $db, $admin_file;
	if($view == 4) { $ingroups = implode("-",$groups); }
    if($view < 4) { $ingroups = ""; }
    if (!empty($url)) {
...
/*****************************************************/
	    ...
	    $db->sql_query("update ".$prefix."_blocks set bkey='$bkey', title='$title', content='$content', url='$url', bposition='$bposition', weight='$weight', active='$active', refresh='$refresh', blanguage='$blanguage', blockfile='$blockfile', view='$view', groups='$ingroups', subscription='$subscription', display='$display' where bid='$bid'");
/*****************************************************/
/* Addon - Conditional Blocks v.1.1.1            END */
/*****************************************************/
...
/* Addon - Conditional Blocks v.1.1.1          START */
/*****************************************************/
    BlocksAdd($title, $content, $url, $bposition, $active, $refresh, $headline, $blanguage, $blockfile, $view, $groups, $expire, $action, $subscription, $display);
/*****************************************************/
/* Addon - Conditional Blocks v.1.1.1            END */
/*****************************************************/
...
/* Addon - Conditional Blocks v.1.1.1          START */
/*****************************************************/
    BlocksEditSave($bid, $bkey, $title, $content, $url, $oldposition, $bposition, $active, $refresh, $weight, $blanguage, $blockfile, $view, $groups, $expire, $action, $subscription, $display);
/*****************************************************/
/* Addon - Conditional Blocks v.1.1.1            END */
/*****************************************************/    
