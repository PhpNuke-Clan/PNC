...
function ShoutBoxAdminMenu($ShoutMenuOptionActive) {
	...
	echo "<b>"._SB_LAYOUT."</b></td></tr></table></td>";
	...
	echo "<b>"._SB_THEMEING."</b></td></tr></table></td>";
	...
	echo "<b>"._SB_PERMISSIONS."</b></td></tr></table></td>";
	...
	echo "<b>"._SB_EMOTICONS."</b></td></tr></table></td>";
	...
	echo "<b>"._SB_CENSOR."</b></td></tr></table></td>";
...
function manageShouts($page, $pruned) {
	...
	if ($pruned > 0) {
		echo "<div align=\"center\" class=\"content\"><b>$pruned</b> "._SB_SHOUTSPRUNED."</div><br />";
	}
	...
	if ($TSrows > 0) {
		echo "<br /><br /><b>NOTE:</b> The prune feature only works on shouts that use the new unix timestamp format. You currently have <b>$TSrows</b> shouts that are using the old format. Please run the conversion tool that came in the Shout_Box.zip to update your shouts.";
	}
	CloseTable();
	echo "<br />";
	OpenTable();
...
function ShoutBoxLayout() {
	...
	echo "<p align=\"center\" class=\"content\"><b>"._SB_VERSION." $row[version]</b></p>";
	if ($v > $row['version']) {
...
function ShoutBoxAdminMonitor() {
	...
	echo "<p align=\"center\" class=\"content\"><b>"._SETUPANDSECURITY."</b></p>";
	$SBhealthCount = 0;
	...
	if ($SBhealthCount == 0) { echo "<b>"._SBEXCELLENT."</b>/"._SBMARGINAL."/"._SBCRITICAL."</p>"; }
	elseif ($SBhealthCount == 1) { echo ""._SBEXCELLENT."/<b>"._SBMARGINAL."</b>/"._SBCRITICAL."</p>"; }
	elseif ($SBhealthCount == 2) { echo ""._SBEXCELLENT."/"._SBMARGINAL."/<b>"._SBCRITICAL."</b></p>"; }
...
function shoutHealth($SBhealthCount) {
	...
	if (file_exists($filename) != TRUE) { echo "<p class=\"content\"><b>Missing security file:</b> $filename&nbsp;&nbsp;<b>Recommendation:</b> This security file is used to keep individuals from browsing the backend of your website. Its a 0 byte size file, which gives them a blank screen if they try to browse the directory it is in. You can usually find this file within another directory of your installation of PHP-Nuke and just copy it into this directory.</p>"; }
	$filename = 'images/admin/shoutbox.gif';
	if (file_exists($filename) != TRUE) { echo "<p class=\"content\"><b>Missing image file:</b> $filename&nbsp;&nbsp;<b>Recommendation:</b> This image file is used in the PHP-Nuke admin area. It is a good idea to have this uploaded, even if you are using the admin area without images, because you may change your mind later and use admin images. You can obtain this image from the Shout Box installation zip file.</p>"; }

	// Level 2 checks (Critical)
	$filename = 'modules/Shout_Box';
	if (is_dir($filename) != TRUE) { echo "<p class=\"content\"><b>Missing critical directory:</b> $filename&nbsp;&nbsp;<b>Recommendation:</b> This directory contains the Shout Box module files. Without it, half of the Shout Box is unavailable for use by the people on your website. Create this directory using cpanel, FTP, or other method.</p>"; }
	$filename = 'shoutbox.js';
	if (file_exists($filename) != TRUE) { echo "<p class=\"content\"><b>Missing critical file:</b> $filename&nbsp;&nbsp;<b>Recommendation:</b> This file contains the Shout Box JavaScript code. It is critical to the operation of this Shout Box. You can obtain this file from the Shout Box installation zip file. Then install it into your PHP-Nuke root folder where mainfile.php is.</p>"; }
	$filename = 'modules/Shout_Box/copyright.php';
	if (file_exists($filename) != TRUE) { echo "<p class=\"content\"><b>Missing critical file:</b> $filename&nbsp;&nbsp;<b>Recommendation:</b> This file contains the Shout Box copyright information. It is both legally required and critical to the success of this Shout Box and OurScripts.net. You can obtain this file from the Shout Box installation zip file.</p>"; }
	$filename = 'modules/Shout_Box/index.php';
	if (file_exists($filename) != TRUE) { echo "<p class=\"content\"><b>Missing critical file:</b> $filename&nbsp;&nbsp;<b>Recommendation:</b> This file contains the Shout Box shout history. Without it, half of the Shout Box features are unavailable for use by the people on your website. You can obtain this file from the Shout Box installation zip file.</p>"; }
	$filename = 'modules/Shout_Box/language/lang-english.php';
	if (file_exists($filename) != TRUE) { echo "<p class=\"content\"><b>Missing critical file:</b> $filename&nbsp;&nbsp;<b>Recommendation:</b> This file contains the Shout Box shout history english language definitions. Even if you are not using them at this time, in the future, you may go multi-lingual. You can obtain this file from the Shout Box installation zip file.</p>"; }
	$filename = 'blocks/block-Shout_Box.php';
	if (file_exists($filename) != TRUE) { echo "<p class=\"content\"><b>Missing critical file:</b> $filename&nbsp;&nbsp;<b>Recommendation:</b> This file is the core of the Shout Box. Without it, the users on your site will not be able to shout. You can obtain this file from the Shout Box installation zip file.</p>"; }
	$filename = 'modules/Shout_Box/images/paypal.gif';
	if (file_exists($filename) != TRUE) { echo "<p class=\"content\"><b>Missing critical image:</b> $filename&nbsp;&nbsp;<b>Recommendation:</b> This image is part of the Shout Box copyright. It is both legally required and critical to the success of this Shout Box and OurScripts.net. You can obtain this file from the Shout Box installation zip file.</p>"; }
	$sql = "select * from ".$prefix."_shoutbox_shouts";
	$result = $db->sql_query($sql);
	if ($result == NULL) { echo "<p class=\"content\"><b>Missing critical SQL table:</b> ".$prefix."_shoutbox_shouts&nbsp;&nbsp;<b>Recommendation:</b> Upload the Shout Box SQL installer (SB_SQL_installer.php) and run it to repair the SQL tables. You can obtain this file from the Shout Box installation zip file in the SQL directory. For security reasons, once repaired, delete the SQL installer from your server.</p>"; }
	$sql = "select * from ".$prefix."_shoutbox_censor";
	$result = $db->sql_query($sql);
	if ($result == NULL) { echo "<p class=\"content\"><b>Missing critical SQL table:</b> ".$prefix."_shoutbox_censor&nbsp;&nbsp;<b>Recommendation:</b> Upload the Shout Box SQL installer (SB_SQL_installer.php) and run it to repair the SQL tables. You can obtain this file from the Shout Box installation zip file in the SQL directory. For security reasons, once repaired, delete the SQL installer from your server.</p>"; }
	$sql = "select * from ".$prefix."_shoutbox_conf";
	$result = $db->sql_query($sql);
	if ($result == NULL) { echo "<p class=\"content\"><b>Missing critical SQL table:</b> ".$prefix."_shoutbox_conf&nbsp;&nbsp;<b>Recommendation:</b> Upload the Shout Box SQL installer (SB_SQL_installer.php) and run it to repair the SQL tables. You can obtain this file from the Shout Box installation zip file in the SQL directory. For security reasons, once repaired, delete the SQL installer from your server.</p>"; }
	$sql = "select * from ".$prefix."_shoutbox_date";
	$result = $db->sql_query($sql);
	if ($result == NULL) { echo "<p class=\"content\"><b>Missing critical SQL table:</b> ".$prefix."_shoutbox_date&nbsp;&nbsp;<b>Recommendation:</b> Upload the Shout Box SQL installer (SB_SQL_installer.php) and run it to repair the SQL tables. You can obtain this file from the Shout Box installation zip file in the SQL directory. For security reasons, once repaired, delete the SQL installer from your server.</p>"; }
	$sql = "select * from ".$prefix."_shoutbox_emoticons";
	$result = $db->sql_query($sql);
	if ($result == NULL) { echo "<p class=\"content\"><b>Missing critical SQL table:</b> ".$prefix."_shoutbox_emoticons&nbsp;&nbsp;<b>Recommendation:</b> Upload the Shout Box SQL installer (SB_SQL_installer.php) and run it to repair the SQL tables. You can obtain this file from the Shout Box installation zip file in the SQL directory. For security reasons, once repaired, delete the SQL installer from your server.</p>"; }
	$sql = "select * from ".$prefix."_shoutbox_ipblock";
	$result = $db->sql_query($sql);
	if ($result == NULL) { echo "<p class=\"content\"><b>Missing critical SQL table:</b> ".$prefix."_shoutbox_ipblock&nbsp;&nbsp;<b>Recommendation:</b> Upload the Shout Box SQL installer (SB_SQL_installer.php) and run it to repair the SQL tables. You can obtain this file from the Shout Box installation zip file in the SQL directory. For security reasons, once repaired, delete the SQL installer from your server.</p>"; }
	$sql = "select * from ".$prefix."_shoutbox_nameblock";
	$result = $db->sql_query($sql);
	if ($result == NULL) { echo "<p class=\"content\"><b>Missing critical SQL table:</b> ".$prefix."_shoutbox_nameblock&nbsp;&nbsp;<b>Recommendation:</b> Upload the Shout Box SQL installer (SB_SQL_installer.php) and run it to repair the SQL tables. You can obtain this file from the Shout Box installation zip file in the SQL directory. For security reasons, once repaired, delete the SQL installer from your server.</p>"; }
	$filename = 'SB_SQL_installer.php';
	$sql = "select * from ".$prefix."_shoutbox_version";
	$result = $db->sql_query($sql);
	if ($result == NULL) { echo "<p class=\"content\"><b>Missing critical SQL table:</b> ".$prefix."_shoutbox_version&nbsp;&nbsp;<b>Recommendation:</b> Upload the Shout Box SQL installer (SB_SQL_installer.php) and run it to repair the SQL tables. You can obtain this file from the Shout Box installation zip file in the SQL directory. For security reasons, once repaired, delete the SQL installer from your server.</p>"; }
	if (file_exists($filename) == TRUE) { echo "<p class=\"content\"><b>SQL installer for Shout Box exists:</b> $filename&nbsp;&nbsp;<b>Recommendation:</b> For security reasons, once you have finished installing, upgrading, or repairing the Shout Box SQL tables, delete the SQL installer from your server.</p>"; }
	$filename = 'shout_box_sql_install.php';
	if (file_exists($filename) == TRUE) { echo "<p class=\"content\"><b>SQL installer for Shout Box exists:</b> $filename&nbsp;&nbsp;<b>Recommendation:</b> For security reasons, delete this SQL installer from your server. It is no longer used.</p>"; }
	$filename = 'shout4.6to5.0.php';
	if (file_exists($filename) == TRUE) { echo "<p class=\"content\"><b>SQL installer for Shout Box exists:</b> $filename&nbsp;&nbsp;<b>Recommendation:</b> For security reasons, delete this SQL installer from your server. It is no longer used.</p>"; }
	$filename = 'shout4.0to4.5.php';
	if (file_exists($filename) == TRUE) { echo "<p class=\"content\"><b>SQL installer for Shout Box exists:</b> $filename&nbsp;&nbsp;<b>Recommendation:</b> For security reasons, delete this SQL installer from your server. It is no longer used.</p>"; }
	...
	$numrowsCheck = $db->sql_numrows($resultCheck);
	if ($numrowsCheck != 1) {
		echo "<p class=\"content\"><b>Shout Box block not added:</b> block-Shout_Box.php&nbsp;&nbsp;<b>Recommendation:</b> You have not added the Shout Box block into PHP-Nuke. Click the Blocks icon in the admin area, scroll down the 'Add a new block' area. In the filename drop down, choose block-Shout_Box.php. Set 'Who can view this' as 'All Visitors', and click the 'Create Block' button.</p>";
	}
	$rowCheck = $db->sql_fetchrow($resultCheck);
	if ($rowCheck['active'] != 1) {
		echo "<p class=\"content\"><b>Shout Box block not active:</b> block-Shout_Box.php&nbsp;&nbsp;<b>Recommendation:</b> Click the Blocks icon in the admin area, scroll down to the 'Blocks Administration' area. In the Functions column and Shout Box row, choose 'Activate'. In the following page, choose 'Yes' to activate the block.</p>";
	}
	if (($rowCheck['view'] != 0) AND ($rowCheck['view'] != 1)) {
		echo "<p class=\"content\"><b>Shout Box block not publicly viewable:</b> block-Shout_Box.php&nbsp;&nbsp;<b>Recommendation:</b> Click the Blocks icon in the admin area, scroll down to the 'Blocks Administration' area. In the Functions column and Shout Box row, choose 'Edit'. In the following page, set 'Who can view this' as 'All Visitors', and click the 'Save Block' button.</p>";
	}
	$sqlCheck = "select * from ".$prefix."_modules where title='Shout_Box'";
	$resultCheck = $db->sql_query($sqlCheck);
	$numrowsCheck = $db->sql_numrows($resultCheck);
	if ($numrowsCheck != 1) {
		echo "<p class=\"content\"><b>Shout Box module not added:</b> block-Shout_Box.php&nbsp;&nbsp;<b>Recommendation:</b> You have not added the Shout Box module into PHP-Nuke. Upload the files into the modules directory. You can obtain the files from the Shout Box installation zip file. Once they are uploaded, Click the Modules icon in PHP-Nuke administration. In the Functions column and Shout Box row, choose 'Activate'. In the Functions column and Shout Box row, choose 'Edit'. In the following page, set 'Who can view this' as 'All Visitors', and click the 'Save Changes' button.</p>";
	}
	$rowCheck = $db->sql_fetchrow($resultCheck);
	if ($rowCheck['active'] != 1) {
		echo "<p class=\"content\"><b>Shout Box module not active:</b> block-Shout_Box.php&nbsp;&nbsp;<b>Recommendation:</b> Click the Modules icon in PHP-Nuke administration. In the Functions column and Shout Box row, choose 'Activate'.</p>";
	}
	if (($rowCheck['view'] != 0) AND ($rowCheck['view'] != 1)) {
		echo "<p class=\"content\"><b>Shout Box module not publicly viewable:</b> block-Shout_Box.php&nbsp;&nbsp;<b>Recommendation:</b> Click the Modules icon in PHP-Nuke administration. In the Functions column and Shout Box row, choose 'Edit'. In the following page, set 'Who can view this' as 'All Visitors', and click the 'Save Changes' button.</p>";
	}
...
function ShoutBoxThemeing() {
	...
	ShoutBoxAdminMenu($ShoutMenuOptionActive);
	OpenTable();
	echo "<p align=\"center\" class=\"content\"><b>"._SB_THEMECOLORING."</b></p>";
	...
	echo "<tr style=\"background-color: $rowColor[menuColor1];\"><td align=\"center\" width=\"15%\" nowrap=\"nowrap\"><b>"._SB_THEME."</b></td><td align=\"center\" nowrap=\"nowrap\" width=\"17%\"><b>"._SHOUTBOX." 1</b></td><td align=\"center\" nowrap=\"nowrap\" width=\"17%\"><b>"._SHOUTBOX." 2</b></td><td align=\"center\" nowrap=\"nowrap\" width=\"17%\"><b>"._SB_BORDER."</b></td><td align=\"center\" nowrap=\"nowrap\" width=\"17%\"><b>"._SB_MENU."/"._SB_ROW." 1</b></td><td align=\"center\" nowrap=\"nowrap\" width=\"17%\"><b>"._SB_MENU."/"._SB_ROW." 2</b></td></tr>";
	...
	echo "<br />";
	OpenTable();
	echo "<p align=\"center\" class=\"content\"><b>"._SB_THEMEIMAGES."</b></p>";

	echo "<form name=\"shoutadmin11\" action=\"\" method=\"post\" style=\"margin-bottom: 0px; margin-top: 0px;\">";
	echo "<table align=\"center\" cellpadding=\"0\" cellspacing=\"0\" border=\"0\" width=\"95%\"><tr><td style=\"background-color: $rowColor[border];\"><table cellpadding=\"3\" cellspacing=\"1\" border=\"0\" width=\"100%\">";

	echo "<tr style=\"background-color: $rowColor[menuColor1];\"><td align=\"center\" width=\"15%\" nowrap=\"nowrap\"><b>"._SB_THEME."</b></td><td align=\"center\" nowrap=\"nowrap\" width=\"17%\"><b>"._SB_BOXARROWS."</b></td><td align=\"center\" nowrap=\"nowrap\" width=\"17%\"><b>"._SB_BOXBACKGROUND."</b></td><td>&nbsp;</td></tr>";
