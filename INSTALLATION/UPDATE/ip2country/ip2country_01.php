...
mysql_query("DELETE FROM `".$prefix."_nsnst_ip2country`");

$wfilename = "ip2country/data/ip2country_1.data";
if(file_exists($wfilename)) {
  ...
  while (!feof($wfread)) {
    ...
    $ip_c2c = ereg_replace ("\n", "", $ip_c2c);
    if($ip_lo > "") {
      $city_title = addslashes($city_title);
      $city_dnam = addslashes($city_dnam);
      $datainserted = False;
