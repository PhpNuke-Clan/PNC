...
/*****************************************************/
/* Forum -Advanced BBCode Box MOD v5.1.0    START */
/*****************************************************/
	...
	$bbcode_tpl['expand_caption_open'] = str_replace('{CAPTION}', '\\1', $bbcode_tpl['expand_caption_open']);
	//$bbcode_tpl['expand_caption_open'] = str_replace('{RAND_NUM}', $uid, $bbcode_tpl['expand_caption_open']);
	//$bbcode_tpl['expand_open'] = str_replace('{RAND_NUM}', $uid, $bbcode_tpl['expand_open']);

/*****************************************************/
/* Forum -Advanced BBCode Box MOD v5.1.0    END */
/*****************************************************/
...
function bbencode_second_pass($text, $uid)
{
	...
	$text = preg_replace("/\[quote:$uid=\"(.*?)\"\]/si", $bbcode_tpl['quote_username_open'], $text);
		...
		// [EXPAND] or [EXPAND=""] and [/EXPAND] for posting replies with expand, or just for expanding stuff.
		$text = str_replace("[expand:$uid]", $bbcode_tpl['expand_open'], $text);
		$text = str_replace("[/expand:$uid]", $bbcode_tpl['expand_close'], $text);
	$text = preg_replace("/\[expand:$uid=\"(.*?)\"\]/si", $bbcode_tpl['expand_caption_open'], $text);
...
/*****************************************************/
	...
	// [expand]Expand text[/expand] code..
	//$text = str_replace("[expand:$uid]", $bbcode_tpl['expand_open'], $text);
	//$text = str_replace("[/expand:$uid]", $bbcode_tpl['expand_close'], $text);
/*****************************************************/
/* Forum -Advanced BBCode Box MOD v5.1.0    END */
     ...
     while ( list($index, $image_source) = each($images[1]) )
     {
         if ( intval($max_image_width) != 0 )
         {
             ...
             if ( $image_width && $image_width > $max_image_width || empty($image_width) || empty($image_height) )
             {
                 ...
                 $image_replacements[] = '<a name="img_' . $rand . '"><a href="#img_' . $rand . '" onClick="img_popup(\'' . $images[1][$index] . '\', ' . $image_width . ', ' . $image_height . ', ' . $rand  . ');"><img src="' . $images[1][$index] .'" width="' . $max_image_width . '" alt="' . $lang['LIW_click_image'] . '" border="0"></a></a><br /><span class="gensmall">' . $lang['LIW_click_image_explain'] . '</span>';
             }
             else
             {
                 $image_patterns[] = $images[0][$index];
...
// EXPAND] or [EXPAND=""] and [/EXPAND] for posting replies with expand, or just for expanding stuff.
	$text = bbencode_first_pass_pda($text, $uid, '[expand]', '[/expand]', '', false, '');
	$text = bbencode_first_pass_pda($text, $uid, '/\[expand=\\\\&quot;(.*?)\\\\&quot;\]/is', '[/expand]', '', false, '', "[expand:$uid=\\\"\\1\\\"]");
...
/*****************************************************/
	...
	// [font=fonttype]text[/font] code..
	$text = preg_replace("#\[font=(Arial|Arial Black|Century Gothic|Comic Sans MS|Courier New|Georgia|Lucida Console|Microsoft Sans Serif|Symbol|Tahoma|Trebuchet|Times New Roman|Verdana)\](.*?)\[/font\]#si", "[font=\\1:$uid]\\2[/font:$uid]", $text);
	...
	// [expand]Expander[/expand] code..
	//$text = preg_replace("#\[expand\](.*?)\[/expand\]#si", "[expand:$uid]\\1[/expand:$uid]", $text);
/*****************************************************/
/* Forum -Advanced BBCode Box MOD v5.1.0    END */
/*****************************************************/
               ...
               if ($close_pos = strpos($text, '&quot;]', $curr_pos + 14))
               {
                  if (strpos(substr($text, $curr_pos + 14, $close_pos - ($curr_pos + 14)), '[quote') === false)
                  {
                     $possible_start = substr($text, $curr_pos, $close_pos - $curr_pos + 7);
								...
								if( preg_match('#\[expand=\\\&quot;#si', $possible_start, $match) && !preg_match('#\[expand=\\\&quot;(.*?)\\\&quot;\]#si', $possible_start) )
								{
									// OK we are in a quote tag that probably contains a ] bracket.
									// Grab a bit more of the string to hopefully get all of it..
									if ($close_pos = strpos($text, '&quot;]', $curr_pos + 15))
									{
										if (strpos(substr($text, $curr_pos + 15, $close_pos - ($curr_pos + 15)), '[expand') === false)
										{
											$possible_start = substr($text, $curr_pos, $close_pos - $curr_pos + 8);
										}
									}
				}
