...
/************************************************************************************/
			...
			if (eregi(".swf",$som_image) || $som_name=="" || $som_name==" " ||$som_name=="&nbsp;" ||$som_name=="&amp;nbsp;") { //////////////////// support des fichiers FLASH - par MAC06 -+- marcoledingue : ajout du second check, qui permet d'avoir des catégories avec un nom vide. //////////////////////////
	...
	if ($general_dynamic==1 && $detectMozilla!=1) { // on va réenrouler toutes les catégories, sauf celle contenant le module affiché sur la page
		...
		if (isset($keyouvert)) { // on ne réenroule pas les sublevels qui vont jusqu'au module affiché sur la page (on laisse l'arborescence du sublevel).
			...
			$ferme_sublevels = ereg_replace("$aenlever_sublevels", "" , $ferme_sublevels);
			$j=$keyouvert;
...
if ( $showadmin==1 && $is_admin===1 && $horizontal!=1) {
	...
	while ($row = $db->sql_fetchrow($result)) {
		...
		$mn_title2 = (!$custom_title) ? ereg_replace("_", " ", $mn_title) : $custom_title;
