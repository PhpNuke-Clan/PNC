...
// get config
include($vwar_root . "includes/_config.inc.php");
...
if ( !defined("VWAR_GLOBALS_GRABBED") )
{
        ...
        function regGlobals ($array, &$target_array)
        {
                ...
                foreach ($array AS $key => $value)
                {
                        ...
                        // we don't want the root path to be changed!
                        if ($key != "vwar_root" &&  $key != "vwar_xroot" && $key != "vwar_root2"){
...
{
        $str[0] = strtolower ( strtr( $str[0], "ÀÁÂÃÄÅÇÈÉÊËÌÍÎÏÑÒÓÔÕÖØÙÚÛÜÝ", "àáâãäåçèéêEûMEòóôõöøùúûüý" ) );
        return $str;
}
## -------------------------------------------------------------------------------------------------------------- ##
