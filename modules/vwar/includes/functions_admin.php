...
/* #####################################################################################
 *
 * $Id: functions_admin.php,v 1.68 2005/11/20 14:33:00 iron Exp $
 *
...
if (@file_exists("../../../install_vwar.php"))
{
	$diemessage = "<p style=\"FONT-FAMILY:verdana,tahoma,arial,sans-serif;FONT-SIZE:13px;\"><b>Please delete install_vwar.php!</b></p>\n";
	// let's die...
	die($diemessage);
}
...
## -------------------------------------------------------------------------------------------------------------- ##
function createWarMail($groups,$warid,$type,$vwarid)
{
...
function getTextRestrictions ($form="vwarform",$textfield="warinfo",$bgcolor="firstalt",$afterbbcode=1, $createclicks=1, $pre="") {
	...
	global $vwartpl,$smiliecode,$htmlcode,$bbcode,${$allowsmilies2},${$allowhtml2},${$allowbbcode2},$clickable_bbcode,
		$clickable_smilies,$nextcolor,$vwarmod,$vwarmod;
