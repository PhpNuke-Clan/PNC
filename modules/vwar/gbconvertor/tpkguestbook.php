<?php
 ...
 /* #####################################################################################
  *
  * $Id: tpkguestbook.php,v 1.1 2004/02/21 22:49:59 frag Exp $
  * TPK Guestbook 3.1.1 - Convertor for vBook 2.0
  *
...
if (!isset($step))
{
	...
	<table border="0" align="center" width="100%" cellpadding="5" cellspacing="0" class="tblborder">
		...
		<tr>
			...
			<td class="firstalt"><input type="text" name="settings[tableentries]" size="20" value="nuke_nukebook" class="input"></td>
		</tr>
		<tr>
...
if ($step == 1)
{
    ...
    $result = $sourcedb->query("
        SELECT id,name,email,url,comment,date,host
    ...
    {
        $vwardb->query("
            INSERT INTO vwar".$n."_gbentries (ip,dateline,guesthomepage,guestemail,guestname,text) VALUES
                ('".$cache[$i]["host"]."','".$cache[$i]["date"]."',
				'".$cache[$i]["url"]."','".$cache[$i]["email"]."',
				'".$cache[$i]["name"]."','".$cache[$i]["comment"]."')");
    }
