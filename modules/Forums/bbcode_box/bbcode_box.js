...
expand_help = 'Expander: [expand]text[/expand] or [expand="caption"]text[/expand]';
...
function BBCexpand() {
	...
	if ((clientVer >= 4) && is_ie && is_win) {
		theSelection = document.selection.createRange().text;
		if (theSelection != '') {
		var enterTITLE = prompt("Enter the title.","title");
		if (!enterTITLE)  {
		FoundErrors += " You didn't write a title.";
		}
		if (FoundErrors)  {
		alert("Error:"+FoundErrors);
		return;
		}
		document.selection.createRange().text = "[expand=\""+enterTITLE+"\"]" + theSelection + "[/expand]";
		document.post.message.focus();
		return;
		}
	}
	...
	if (Expander == 0) {
		...
		ToAdd = "[expand=\""+enterTITLE+"\"]";
		document.expand.src = "modules/Forums/bbcode_box/images/expand1.gif";
		Expander = 1;
	} else {
		ToAdd = "[/expand]";
		document.expand.src = "modules/Forums/bbcode_box/images/expand.gif";
		Expander = 0;
	}
	PostWrite(ToAdd);
}
