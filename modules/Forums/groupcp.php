...
//Header("Location: modules.php?name=Groups");
...
//********* BEGIN MODIFED by Duck for NSN and PHP Group syncronization *******//
$uid = $userdata['user_id'];
$username = $userdata['username'];
                				$sql = "SELECT `gid` FROM `".$prefix."_nsngr_groups` WHERE `phpBB`='$group_id'";
  				   				 if ( !($result = $db->sql_query($sql)) )
  				         		  {
  				                   message_die(GENERAL_ERROR, "Error Retreiving NSN Group Info", "", __LINE__, __FILE__, $sql);
  				     			   }
 							 list($gid) = $db->sql_fetchrow($result);
 							$stime = time();
							 $sql = "INSERT INTO ".$prefix."_nsngr_users VALUES ('$gid','$uid','$username','0','0','$stime','0')";
  				                if ( !($result = $db->sql_query($sql)) )
  				                		{
  				                     		   message_die(GENERAL_ERROR, 'Could not INSERT NSN group memebership data', '', __LINE__, __FILE__, $sql);
              							  }
 //********* END MODIFED by Duck for NSN and PHP Group syncronization *******//
...
else if ( isset($HTTP_POST_VARS['unsub']) || isset($HTTP_POST_VARS['unsubpending']) && $group_id )
{
	...
	else if ( $sid !== $userdata['session_id'] )
	{
        ...
        if ( $confirm )
        {
                ...
                if ( !($result = $db->sql_query($sql)) )
                {
                        message_die(GENERAL_ERROR, 'Could not delete PHP group memebership data', '', __LINE__, __FILE__, $sql);
                }
//********* BEGIN MODIFED by Duck for NSN and PHP Group syncronization *******//
                $sql = "SELECT `gid` FROM `".$prefix."_nsngr_groups` WHERE `phpBB`='$group_id'";
				    if ( !($result = $db->sql_query($sql)) )
...
list($gid) = $db->sql_fetchrow($result);
                 ...
                 $sql = "DELETE FROM " . $prefix . "_nsngr_users
				                        WHERE uid = " . $userdata['user_id'] . "
								AND gid = $gid";
				                if ( !($result = $db->sql_query($sql)) )
				                {
				                        message_die(GENERAL_ERROR, 'Could not delete NSN group memebership data', '', __LINE__, __FILE__, $sql);
                }
//*********END MODIFED by Duck for NSN and PHP Group syncronization *******//
...
//******************
						$uid = $row['user_id'];
                                        if ( !$db->sql_query($sql) )
                                        {
                                                message_die(GENERAL_ERROR, 'Could not add user to phpBB group', '', __LINE__, __FILE__, $sql);
                                        }
