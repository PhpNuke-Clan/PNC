...
if ($multilingual == 1) {
	...
	if (empty($sid)) { $sid = 0; } else { $sid = intval($sid); }
	...
	if ($type=="stories" OR !$type) {
		...
		if ($nrows>0) {
			while($row5 = $db->sql_fetchrow($result5)) {
                ...
                $furl = "modules.php?name=News&op=NEArticle&sid=$sid";
...
*/
		...
		if ($nrows>0) {
			while($row8 = $db->sql_fetchrow($result8)) {
			    ...
			    $furl = "modules.php?name=News&amp;op=NEArticle&amp;thold=-1&amp;mode=flat&amp;order=1&amp;sid=$sid#$tid";
				if(!$name) {
					$name = "$anonymous";
			    } else {
