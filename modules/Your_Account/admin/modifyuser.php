...
if($db->sql_numrows($result) > 0) {
    ...
    //}
    //if ($Version_Num > 6.9) {
        echo "<tr><td bgcolor='$bgcolor2'>"._YA_POINTS.":</td><td><input type='test' name='chng_points' value='".$chnginfo['points']."'></td></tr>\n";
    //}
	
    $signature = $chnginfo['user_sig'];
	$signature_bbcode_uid = $chnginfo['user_sig_bbcode_uid'];
	$signature = ($signature_bbcode_uid != '') ? preg_replace("/:(([a-z0-9]+:)?)$signature_bbcode_uid(=|\])/si", '\\3', $signature) : $signature;
	
	echo "<tr><td bgcolor='$bgcolor2' valign='top'>"._SIGNATURE.":</td><td><textarea name='chng_user_sig' rows='6' cols='45'>".$signature."</textarea></td></tr>\n";
