	...
	//if   ($Version_Num >= 7.5)  { 
       $add_pwd = md5($add_pwd); 
        for ($i=0; $i < sizeof($auth_modules); $i++) { 
         ...
         $add_radminblocker = check_html($add_radminblocker, 'nohtml');
        $result = $db->sql_query("insert into " . $prefix . "_authors values ('$add_aid', '$add_name', '$add_url', '$add_email', '$add_password', '0', '$add_radminsuper', '$add_admlanguage', '$add_radminblocker')");
    /*} 
