...
//--------------------------------------------
		...
		$signature_bbcode_uid = $userinfo[user_sig_bbcode_uid];
		$signature = ($signature_bbcode_uid != '') ? preg_replace("/:(([a-z0-9]+:)?)$signature_bbcode_uid(=|\])/si", '\\3', $signature) : $signature;
				
        echo "<td bgcolor='$bgcolor3'><textarea wrap='virtual' cols='50' rows='5' name='user_sig'>$signature</textarea><br>"._255CHARMAX."</td></tr>";
        ...
        if (ereg("http", $userinfo['user_avatar'])) {
                        ...
                        echo "<td bgcolor='$bgcolor3' align=center>"._YA_CURRAV."<BR><IMG alt=\"\" src=\"$userinfo[user_avatar]\" width=\"40\" height=\"50\"></td></tr>";
