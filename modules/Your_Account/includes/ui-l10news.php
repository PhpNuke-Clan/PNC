...
if (!defined('CNBYA')) { echo "CNBYA protection"; exit; }
    ...
    if ($articlecomm == 1) {
        ...
        if (($db->sql_numrows($result6) > 0)) {
            ...
            while($row6 = $db->sql_fetchrow($result6)) {
                ...
                echo "<li><a href=\"modules.php?name=News&op=NEArticle&thold=-1&mode=flat&order=0&sid=$sid#$tid\">$subject</a><br>";
            }
            CloseTable();
        }
    }
    ...
    if (($db->sql_numrows($result7) > 0)) {
        ...
        while($row7 = $db->sql_fetchrow($result7)) {
            $sid = $row7[sid];
            $title = $row7[title];
            echo "<li><a href=\"modules.php?name=News&op=NEArticle&sid=$sid\">$title</a><br>";
