...
$topics = 1;
automated_news();
if($new_topic == 0 OR empty($new_topic)) { header("Location: $form_link"); }
$topic = intval($new_topic);
...
if($totalarticles > 0) {
  ...
  while($artinfo = $db->sql_fetchrow($result)) {
    ...
    $artinfo['title'] = ne_check_html(ne_convert_text($artinfo['title']), 0);
  ...
  if($articlepages!=1 && $articlepages!=0) {
    ...
    echo "<form action='".$module_link."op=NETopicList' method='post'>\n";
  ...
  OpenTable();
  echo "<center class='title'>"._NE_NOARTICLETOPIC."</center><br>";
  CloseTable();
}
include("footer.php");
