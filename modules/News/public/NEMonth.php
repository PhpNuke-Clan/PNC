...
$ddate = $year."-".$month."-01";
$udate = strtotime($ddate);
$pagetitle = ": "._NE_STORIESARCHIVE.": ".date("M Y", $udate);
$perpage = 50;
...
include("header.php");
title(_NE_STORIESARCHIVE.": ".date("M Y", $udate));
if(is_user($user)) { getusrinfo($user); }
OpenTable();
echo "<center>[ <a href='".$module_link."op=NEArchive'>"._NE_ARCHIVESINDEX."</a> | <a href='".$module_link."op=NEAll'>"._NE_SHOWALLSTORIES."</a> ]</center><br>";
$numrows = $db->sql_numrows($db->sql_query("SELECT * FROM `".$prefix."_stories` WHERE `time` >= '$year-$month-01 00:00:00' AND `time` <= '$year-$month-31 23:59:59'"));
...
$result = $db->sql_query("SELECT * FROM `".$prefix."_stories` WHERE `time` >= '$year-$month-01 00:00:00' AND `time` <= '$year-$month-31 23:59:59' ORDER BY `sid` DESC LIMIT $min,$perpage");
while ($row = $db->sql_fetchrow($result)) {
  ...
  $actions .= "&nbsp;<a href='".$module_link."op=NEPrint&amp;sid=$sid' target='_blank'><img src='modules/$module_name/images/print.png' border=0 alt='"._NE_PRINTER."' title='"._NE_PRINTER."' width='15' height='16'></a>";
  $actions .= "&nbsp;<a href='".$module_link."op=NEPortable&amp;sid=$sid' target='_blank'><img src='modules/$module_name/images/pdf.png' border=0 alt='"._NE_PDF."' title='"._NE_PDF."' width='15' height='16'></a>";
  $actions .= "&nbsp;<a href='".$module_link."op=NEFriend&amp;sid=$sid' target='_blank'><img src='modules/$module_name/images/friend.png' border=0 alt='"._NE_FRIENDSEND."' title='"._NE_FRIENDSEND."' width='15' height='16'></a>";
  if($score != 0) {
  ...
  if($catid == 0) {
    ...
    $res = $db->sql_query("SELECT `title` FROM `".$prefix."_stories_cat` WHERE `catid`='$catid'");
    $row_res = $db->sql_fetchrow($res);
    $db->sql_freeresult($res);
    $cat_title = ne_check_html(ne_convert_text($row_res['title']), 0);
...
echo "</table>\n";
  ...
  if($articlepages > 1) {
    ...
    echo "<form action='".$module_link."op=NEMonth' method='post'>\n";
