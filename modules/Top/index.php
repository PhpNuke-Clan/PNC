...
if ($db->sql_numrows($result) > 0) {
    ...
    $counter = intval($row['counter']);
        if($counter>0) {
    	    echo "<strong><big>&middot;</big></strong>&nbsp;$lugar: <a href=\"modules.php?name=News&amp;op=NEArticle&amp;sid=$sid\">$title</a> - ($counter "._READS.")<br>\n";
	    $lugar++;
	}
    }
...
if ($db->sql_numrows($result2) > 0) {
    ...
    $ratings = intval($row2['ratings']);
        if($ratings>0) {
    	    echo "<strong><big>&middot;</big></strong>&nbsp;$lugar: <a href=\"modules.php?name=News&amp;op=NEArticle&amp;sid=$sid\">$title</a> - ($ratings "._LVOTES.")<br>\n";
	    $lugar++;
	}
    }
    echo "</font></td></tr></table><br>\n";
...
if ($db->sql_numrows($result3) > 0) {
	...
	$ratings = intval($row3['ratings']);
	    ...
	    $rate = substr($score / $ratings, 0, 4);
    	    echo "<strong><big>&middot;</big></strong>&nbsp;$lugar: <a href=\"modules.php?name=News&amp;op=NEArticle&amp;sid=$sid\">$title</a> - ($rate "._POINTS.")<br>\n";
	    $lugar++;
	}
    }
    echo "</font></td></tr></table><br>\n";
...
if ($articlecomm == 1) {
    ...
    if ($db->sql_numrows($result4) > 0) {
		...
		while ($row4 = $db->sql_fetchrow($result4)) {
		    ...
		    if($comments>0) {
				echo "<strong><big>&middot;</big></strong>&nbsp;$lugar: <a href=\"modules.php?name=News&amp;op=NEArticle&amp;sid=$sid\">$title</a> - ($comments "._COMMENTS.")<br>\n";
				$lugar++;
		    }
		}
		echo "</font></td></tr></table><br>\n";
